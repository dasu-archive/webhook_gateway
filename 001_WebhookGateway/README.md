...
| 항목 | 왜 뺐나 |
| --- | --- |
| 재생 (9절) | ✅ `/admin/replays` — 드라이런 · 범위 지정 · 가드 4개 · 재생 이력. DEAD 를 제자리에서 PENDING 으로 되돌린다 |
...
아래 명령은 이 디렉토리(`001_WebhookGateway/`)에서 실행한다.
...
저장소 루트의 `compose.yaml` 에서 MySQL 만 띄운다. 전체 스택을 컨테이너로 돌리려면
루트에서 `docker compose up -d --build` 를 쓴다 — 그때는 아래 2~5 단계가 필요 없다.
...
```bash
java ../000_Infra/tools/MockConsumer.java 9090
```
...
```bash
../000_Infra/tools/send.sh gh-push topsecret-secret-1234 github
```
...
```bash
# 같은 이벤트 ID 로 20번 쏜다
../000_Infra/tools/send.sh gh-push topsecret-secret-1234 github - 20 --same-id
...
../000_Infra/tools/send.sh gh-push topsecret-secret-1234 github - 200
